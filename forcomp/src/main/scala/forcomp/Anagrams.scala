...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(x => x).map{case(x,y) => (x, y.length)}.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = 
    if (occurrences.isEmpty) List(List())
          ...
          val currentSymbolFreq: Int = occurrences.head._2
          (for {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    y match{
    case i@List(_) => x.toMap.updated(i.head._1,(x.toMap.apply(i.head._1)-i.head._2)).toList.filter(l => l._2 > 0)
    case i::is => subtract(x.toMap.updated(y.head._1,(x.toMap.apply(y.head._1)-y.head._2)).toList.filter(l => l._2 > 0), y.tail)
  }
    ...
    def subAnagrams(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List())
        ...
        for {
          ...
          word <- dictionaryByOccurrences(combination.filter(p => if (p == ()) true else false))
