...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {    
    def helper(c: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      if (!c.isEmpty)
        if(acc.map(_._1) contains c.head) helper(c.tail, acc updated ((acc.map(_._1) indexOf c.head), (c.head, acc(acc.map(_._1) indexOf c.head)._2 + 1)))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x :: Nil => trees
    case _ => (makeCodeTree(trees.head, trees.tail.head) :: trees.tail.tail).sortWith( weight(_) < weight(_) )
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def helper(t: CodeTree, b: List[Bit]): List[Char] = t match{
      ...
      case Leaf(c,_) => c :: helper(tree, b)
      case Fork(l,r,_,_) if (b.head == 1) => helper(r, b.tail) 
      case Fork(l,r,_,_) => helper(l, b.tail)
    }
    helper(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def helper(t: CodeTree, tx: List[Char]): List[Bit] = t match{
      case Fork(l,r,chars,_) if(tx.head == chars.head) => 0 :: helper(l, tx)
      case Fork(l,r,chars,_) => 1 :: helper(r, tx)
      case Leaf(_,_) if (tx.tail.isEmpty) => List()
      case Leaf(_,_) => helper(tree, tx.tail)
    }
    helper(tree,text)
  }
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table(table.map(_._1).indexOf(char))._2
