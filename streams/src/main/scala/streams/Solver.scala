...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.map{case(x,y) => (x,y::history)}.toStream
...
/*           {
    if (initial.isEmpty) Stream()
      ...
      //println(explored.toList.toString)
